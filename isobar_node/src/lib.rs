...
mod buffer {
    use super::*;
    use isobar_core::buffer::{Buffer, ReplicaId};
...
mod editor {
    ...
    use isobar_core::editor::{Editor, render};
    use super::*;
    ...
    fn render<'a>(env: &'a Env, this: Value<'a, Object>, args: &[Value<'a, Any>]) -> Result<Option<Value<'a, Any>>> {
        ...
        let mut js_liens = env.create_array_with_length(frame.lines.len());
        ...
        for (i, selection) in frame.selections.iter().enumerate() {
            ...
            js_selection.set_index(i, js_selection)?;
        }
        js.frame.set_named_property("selections", js_selections)?;
