...
use isobar_core::{self, schema_capnp, App, WindowId};
use isobar_core::messages::{IncomingMessage, OutgoingMessage};
...
impl Server {
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>,
    {
        if server.app.headless() {
            ...
            server.send_response(outgoing, responses);
        }
    }
    ...
    where
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>,
    {
        match (server.app.borrow().peer.headless(), headless) {
            (true, false) => {
                return server.send_response(outgoing, stream::once(Ok(OutgoingMessage::Error {
