...
use isobar_core::{self, Peer, WindowId};
...
impl App {
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>,
    {
        ...
        if app_state.peer.headless() {
            app_state.send_response(outgoing, stream::once(Ok(OutgoingMessage::Error {
            ...
            app_state.send_response(outgoing, responses);
        }
    }
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>,
    {
        match (app_state.borrow().peer.headless(), headless) {
            (true, false) => {
                return app_state.borrow().send_response(outgoing, stream::once(Ok(OutgoingMessage::Error {
        ...
        app_state.borrow_mut().send_response(outgoing, responses);
    }
        ...
        I: 'static + Stream<Item = IncomingMessage, Error = io::Error>,
    {
        ...
        let outgoing_messages = app_state
            .peer
            .window_update(&window_id, height)
