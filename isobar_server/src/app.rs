    ...
    fn open_workspace(&mut self, paths: Vec<PathBuf>) -> Result<(), &'static str> {
        ...
        if let Some(ref mut app_channel) = self.app_channel {
            app_channel
                ...
                .expect("Tried to open workspace with no coneected app");
        }
