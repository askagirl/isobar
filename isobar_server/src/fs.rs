...
impl Tree {
    pub fn new<T: Into<PathBuf>>(path: T) -> Result<Self, &'static str> {
        ...
        let root = isobar_fs::Entry::dir(file_name, false, false);
    ...
    fn populate(path: PathBuf, root: isobar_fs::Entry, updates: Arc<NotifyCell<()>>) {
        thread::spawn(move || {
...
impl isobar_fs::Tree for Tree {
    fn root(&self) -> isobar_fs::Entry {
        self.root.clone()
    }
...
impl isobar_fs::LocalTree for Tree {
    fn path(&self) -> &Path {
        &self.path
    }
    ...
    fn as_tree(&self) -> &isobar_fs::Tree {
        self
    }
}
...
impl isobar_fs::FileProvider for FileProvider {
    fn open(&self, path: &Path) -> Box<Future<Item = Box<isobar_fs::File>, Error = io::Error>> {
        ...
        Box::new(
            rx.then(|result| result.expect("Sender should not be dropped"))
                .map(|file| Box::new(file) as Box<isobar_fs::File>),
        )
    }
}
...
impl isobar_fs::File for File {
    fn id(&self) -> isobar_fs::FileId {
        self.id
    }
