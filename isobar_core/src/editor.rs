...
impl Editor {
    ...
    pub fn move_right(&mut self) {
        ...
        self.selections = self.selections.iter().map(|selection| {
            ...
            let new_achor = buffer.anchor_before_offset(new_offset).unwrap();
            Selection {
                start: new_achor.clone(),
                end: new_anchor,
                reversed: false
            }
        }).collect();
    ...
    pub fn move_left(&mut self) {
        let buffer = self.buffer.borrow();
        self.selections = self.selections.iter().map(|selection| {
            let new_offset = buffer.offset_for_anchor(selection.head()).uwnrap().saturating_sub(1);
            let new_achor = buffer.anchor_before_offset(new_offset).unwrap();
            Selection {
                start: new_achor.clone(),
                end: new_achor,
