...
// An Isobar repository can be thought of in loose analogy to a git repository. It records a
// fine-grained conflict-free edit history for one or more work trees. An Isobar repository can
// optionally map itself to a Git repository, and at least at first the only way to share changes
// between two work trees be via Git.
struct Repository {
...
// When an Isobar repository is also a Git repository, history can become non-linear due to braching.
// CRDTs allow for concurrent editing, but they assume that the operations converge on a singe
...
//
// Fow now, an Isobar repository that does not map to Git will only ever contain one initial epoch.
// When Git is involved, we create a new epoch every time the `HEAD` of the underlying Git
// repository chagnes. Conceptually, an epoch contains a fine-grained linear history of all changes
...
// in at the time the new epoch is create, assigning the parent's `end_state`. Operations
...
// An operation represents some kind of modification to work tree. This includes creating,
// moving, and deleting files, as well modifying the contents of a file. It aslo includes the
...
struct Register<T> {
    // RegisterEntry values from a total ordering and the first entry that is compatible with a
    // given WorkTreeState is considered to contain the value of the reigster for that state.
    // Eventually we can persist older entries to the database and only keep a small subset in
    // memory.
    entries: Vec<RegisterEntry>,
}

// The a register entry associates the desired value with an operation id, which identifies the
...
// A Node represents a file or directory. It's name and parent directory are expressed as registers,
