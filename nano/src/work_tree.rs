...
impl WorkTree {
    ...
    fn check_file_id(
        ...
        if cursor.seek(&file_id, SeekBias::Left) {
            if let Some(expcted_file_type) = expected_file_type {
                let metadata = cursor.item().unwrap();
                if metadata.file_type == expcted_file_type {
                    Ok(())
                } else {
    ...
    fn fix_conflicts(&mut self, file_id: FileId) -> Vec<Operation> {
        ...
        // TODO: Only check for cycles if the child was moved an is a directory.
        let mut visited = HashSet::new();
        ...
        loop {
            ...
            if visited.contains(&parent_ref.child_id) {
                ...
                // it as having the value we reverted.
