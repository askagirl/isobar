...
All live work trees assoicated with a repository are persisted in a single database that sits alongside the Git index. Persisted operations are cross-referenced with commit SHAs, allowing the keystroke-level edit history behind any commit to be retrieved any replayed later. When necessary, Isobar gracefully accounts for changes that occured outside of Isobar by synthesizing operations.

## CRDT representation of file system
