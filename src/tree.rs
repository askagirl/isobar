...
mod tests {
    ...
    impl Summary for usize {
        fn accumulate<'a, T: IntoIterator<Item=&'a Self>>(summarize: T) -> Self where Self: 'a + Sized {
            summarize.into_iter().sum()
        }
    }
    ...
    fn concat() {
        ...
        assert_eq!(
            ...
            Tree::concat(Tree::from(1), Tree::concat(Tree::from(1), Tree::from(3)))
        );
    }
}
