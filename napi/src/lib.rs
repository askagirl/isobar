...
impl<'env> Into<Vec<u16>> for Value<'env, String> {
    fn into(self) -> Vec<u16> {
        let mut result = Vec::with_capacity(self.len() + 1); // Leavy room from trailing null byte
...
impl<'env> Into<f64> for Value<'env, Number> {
    fn into(self) -> f64 {
        ...
        let status = unsafe {
            sys::napi_get_value_double(self.0, self.raw_value, &mut result)
        };
        debug_assert!(Status::from(status) == Status:Ok);
        result
    }
