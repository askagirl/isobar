...
The core of the application is written in Rust. The core is a pure Rust library that lives in `/isobar_core`. The core is then exposed to JavaScript as a native add-on for Node.js in `/isobar_node`. The `isobar_node` library is implemented with an N-API wrapper for Rust located in `/napi`.
