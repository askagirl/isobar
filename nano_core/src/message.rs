...
pub struct UpdateParent<'a> {
    ...
    pub new_parent: Option<FileId<'a>>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(18) => msg.new_parent = Some(r.read_message::<FileId>(bytes)?),
...
impl<'a> MessageWrite for UpdateParent<'a> {
    fn get_size(&self) -> usize {
        ...
        + self.new_parent.as_ref().map_or(0, |m| 1 + sizeof_len((m).get_size()))
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if let Some(ref s) = self.new_parent { w.write_with_tag(18, |w| w.write_message(s))?; }
...
impl<'a> MessageRead<'a> for GlobalTimestamp<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.timestamp.push(r.read_message::<Timestamp>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl<'a> MessageWrite for GlobalTimestamp<'a> {
    fn get_size(&self) -> usize {
        0
        + self.timestamp.iter().map(|s| 1 + sizeof_len((s).get_size())).sum::<usize>()
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        for s in &self.timestamp { w.write_with_tag(10, |w| w.write_message(s))?; }
        Ok(())
    }
}
