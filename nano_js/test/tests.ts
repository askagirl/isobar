...
suite("WorkTree", () => {
  ...
  test("epoch id", async () => {
    ...
    assert.deepStrictEqual(envelope1EpochId, envelope1EpochId);
...
type BaseEntry =
  | NanoBaseEntry & { type: FileType.Directory }
  | NanoBaseEntry & { type: FileType.Text; text: string };
