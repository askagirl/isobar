...
```js
const nano = require("@siberianmh/nano"); // alternatively: `import * as nano from "@siberianmh/nano";`
...
The `rename` method takes a file id to rename, the id of a parent directory, and the file's new name. In the example above, we access the id of the root directory via `getRootFileId()`. If you attempt to rename the file to a name that conflicts with an existing entry in the specified parent, an exception will be thrown.
...
To rename or delete a file, you need access to ids file id. You saw how to obtain the file id for a new file or directory above. There are a couple ways to obtain the id of an existing file. First, it's available in the `fileId` field in each of the objects returned by the `entries` method, described above. If you're rendering a UI based on this information, you could potentially associated this file id with a rendered UI element in some way. If you want to get the file id for a path, you can call `fileIdForPath`.
...
To retrieve a file's original path as it existed in the base commit before any rename operations were applied you can call `basepathForFileId`. This method returns `null` if the file did not exists in the base commit.
...
Each changes in the returned diff has a `start` and `end` based on the current state of the document along with the text that was inserted. You can iterate these changes in order and use the supplied coordinates to apply them to another document.
...
If you're integrating with a text editor, you should capture the work tree's version vector before applying remote operations, then obtain and apply and changes to open buffers via `changesSince` method. Here is a sketch of how this would work.

```js
function applyOps(tree, ops, openEditors) {
  // Perform these steps synchronosuly:
  const baseVersion = tree.getVersion();
  const fixupOps = tree.applyOps();
