...
use isobar_core::{cross_platform, App, ViewId, WindowId, WindowUpdate};
            ...
            .and_then(move |connection| {
                executor
                    .execute(Box::new(
                        outgoing.send_all(
                            connection
                                // TODO: go back to using Vec<u8> for outgoing messages in isobar_core.
...
impl isobar_core::fs::FileProvider for FileProvider {
    fn open(
        &self,
        _: &cross_platform::Path,
    ) -> Box<Future<Item = Box<isobar_core::fs::File>, Error = io::Error>> {
        unimplemented!()
    }
}
