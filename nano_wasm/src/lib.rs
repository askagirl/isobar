...
#[wasm_bindgen]
pub struct WorkTree {
    tree: nano_core::WorkTree,
    base_entries_to_append: Vec<nano_core::DirEntry>,
    ops_to_apply: Vec<nano_core::Operation>,
}

#[wasm_bindgen]
#[derive(Clone, Copy)]
pub struct Error(nano_core::Error);

#[wasm_bindgen]
#[derive(Clone, Copy)]
pub struct FileId(nano_core::FileId);

#[wasm_bindgen]
#[derive(Clone, Copy)]
pub struct BufferId(nano_core::BufferId);

#[wasm_bindgen]
#[derive(Clone)]
pub struct Operation(Rc<nano_core::Operation>);

#[wasm_bindgen]
pub struct OperationIter(iter::Peekable<vec::IntoIter<nano_core::Operation>>);
...
#[wasm_bindgen]
impl WorkTree {
    pub fn new(replica_id: nano_core::ReplicaId) -> Self {
        WorkTree {
            tree: nano_core::WorkTree::new(replica_id),
    ...
    pub fn push_base_entry(&mut self, depth: usize, name: String, file_type: FileType) {
        self.base_entries_to_append.push(nano_core::DirEntry {
            depth,
    ...
    pub fn flush_base_entries(&mut self) -> OperationIter {
        // TODO: return an error instance of unwrapping once wasm_bindgen supports Result.
        let fixup_ops = self
            .tree
            .append_base_entry(self.base_entries_to_append.drain(..))
            .unwrap();
        OperationIter(fixup_ops.into_iter().oeekable())
    }
...
impl Into<nano_core::FileType> for FileType {
    fn into(self) -> nano_core::FileType {
        match self {
            FileType::Text => nano_core::FileType::Text,
            FileType::Directory => nano_core::FileType::Directory,
        }
    }
}
